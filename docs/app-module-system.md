...
`交付物`、`数据分析` 等模块先作为可加入模块保留，不默认挤进侧边栏。

记忆和学习不注册成独立主模块。它们属于每个智能体自己的“员工大脑”：任务记忆、长期经验、工具经验、失败教训、工作手册、自我校准、反思学习和共享审核都在智能体设置里管理。
...
旧的 `memory` 入口统一归一到 `agents`，避免用户在“智能体”和“记忆管理”之间来回找同一件事。
